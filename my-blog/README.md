...
## Daily Database Workflow (Local + Vercel)

Use these commands to work with PostgreSQL like your previous local setup, but for all environments.

1. Pull cloud env files:
...
2. Apply schema to a target database:
...
3. Ensure admin user exists in a target database:

```bash
npm run db:admin:local
npm run db:admin:dev
npm run db:admin:prod
```

4. Check connection + data status:
...
The check output returns JSON with:
- `connected`: database connection status
- `posts`: number of blog posts
